...
1. Create a GitHub repository named `datasetlint`.
...
```bash
./scripts/render-homebrew-formula.sh --owner itamaker --version v0.1.0 > /path/to/homebrew-tap/Formula/datasetlint.rb
```

Commit the rendered file to `https://github.com/itamaker/homebrew-tap` as `Formula/datasetlint.rb`, then users can install with:
