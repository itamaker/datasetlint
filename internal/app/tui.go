...
func runTUI() int {
	model := newTUIModel("datasetlint", "Interactive dataset quality and overlap analysis", buildTUIActions())
...
func buildTUIActions() []tuiAction {
	return []tuiAction{
		{
			name:        "scan",
			description: "Scan train/eval datasets for overlap, duplicates, and conflicts",
			fields: []tuiField{
				{key: "train", label: "Train Path", placeholder: "examples/train.jsonl"},
				{key: "eval", label: "Eval Path", placeholder: "examples/eval.jsonl"},
				{key: "semantic_threshold", label: "Semantic Threshold", placeholder: "0.6", optional: true},
				{key: "max_samples", label: "Max Samples", placeholder: "5", optional: true},
				{key: "strict", label: "Strict Mode", placeholder: "false", optional: true},
				{key: "json", label: "JSON Output", placeholder: "false", optional: true},
			},
			run: func(values map[string]string) (string, error) {
				args := []string{"-train", values["train"], "-eval", values["eval"]}
				if values["semantic_threshold"] != "" {
					args = append(args, "-semantic-threshold", values["semantic_threshold"])
				}
				if values["max_samples"] != "" {
					args = append(args, "-max-samples", values["max_samples"])
				}
				if truthy(values["strict"]) {
					args = append(args, "-strict")
				}
				if truthy(values["json"]) {
					args = append(args, "-json")
				}
				return captureRun(runScan, args)
			},
		},
	}
}
